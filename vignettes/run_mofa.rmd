...
colData(coad_mae_emb) <- metadata(coad_mae_emb) |> 
    dplyr::mutate(patient = TCGAutils::TCGAbarcode(sampleMap(coad_mae_emb)$colname))
...
## Create object and attach metadata for coad 
...
# 1. Extract patient-level metadata from colData
sample_order <- samples_names(mofa_obj)[[1]]

# 2. Build patient-level metadata from colData of the original MAE
#    (Take first row per patient to avoid duplicates)
coldata_all <- as.data.frame(colData(coad_mae_emb))
coldata_all$patient <- TCGAutils::TCGAbarcode(rownames(coldata_all))
patient_meta <- coldata_all[!duplicated(coldata_all$patient), ]
rownames(patient_meta) <- patient_meta$patient

# 3. Create metadata frame with required 'sample' column and all other columns
meta_for_mofa <- patient_meta[sample_order, ]
meta_for_mofa <- data.frame(sample = sample_order, meta_for_mofa, row.names = NULL)

# 4. Attach to MOFA object
samples_metadata(mofa_obj) <- meta_for_mofa

table(meta_for_mofa$patient.tissue_source_site)
```


## Create object and attach metadata for ov 


```{r}
# Create MOFA object from MultiAssayExperiment
mofa_obj <- create_mofa(ov_mae_emb_aligned)
# 1. Get sample order (patient IDs) from the MOFA object
sample_order <- samples_names(mofa_obj)[[1]]

# 2. Build patient-level metadata from colData of ov_mae_emb
coldata_all <- as.data.frame(colData(ov_mae_emb))
coldata_all$patient <- TCGAutils::TCGAbarcode(rownames(coldata_all))
patient_meta <- coldata_all[!duplicated(coldata_all$patient), ]
rownames(patient_meta) <- patient_meta$patient
