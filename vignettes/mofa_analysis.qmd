...
df <- left_join(df, metadata, by = c("sample_id" = "patientID"))
...
cat_summary <- lapply(cat_vars, function(v) {
  ...
  c1 <- paste0(colnames(tab), ": ", as.integer(tab["Cluster1",]), " (", round(100*tab["Cluster1",]/sum(tab["Cluster1",]),1), "%)", collapse="; ")
  c2 <- paste0(colnames(tab), ": ", as.integer(tab["Cluster2",]), " (", round(100*tab["Cluster2",]/sum(tab["Cluster2",]),1), "%)", collapse="; ")
  p <- NA
  test <- NA
...
pathway_vars <- names(df)[sapply(df, is.numeric)]
...
# --- 6. Plot clusters
ggplot(df, aes(x=factor_score, y=projection, color=cluster)) +
...
surv_df <- df %>%
  mutate(
    time = ifelse(
      vital_status.x == 1,
      days_to_death.x,
      pmax(patient.days_to_last_followup,
           patient.days_to_last_known_alive,
           na.rm = TRUE)
    ),
    event = ifelse(vital_status.x == 1, 1, 0)
    
  ) %>%
  filter(!is.na(time), !is.na(event), !is.na(cluster), time > 0) %>%
...
```{r}
# Ensure metadata merged correctly
weights_metadata_df <- get_factors(mofa_object.trained, factors = "all", as.data.frame = TRUE) %>%
  inner_join(metadata_df, by = c("sample" = "patientID"))

weights_metadata_wide <- weights_metadata_df %>%
  select(sample, factor, value, all_of(clin_vars)) %>%
  pivot_wider(names_from = factor, values_from = value)
...
table(weights_metadata_wide$patient.anatomic_neoplasm_subdivision)
# empty df for results
r2_df_mofa_weights <- data.frame()

for (var in clin_vars) {
  for (f in grep("^Factor", names(weights_metadata_wide), value = TRUE)) {
    y <- as.numeric(weights_metadata_wide[[f]])
    x <- weights_metadata_wide[[var]]
