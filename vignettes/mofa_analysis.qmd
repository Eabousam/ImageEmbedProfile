...
```{r load-mofa, include =TRUE, echo = TRUE}

# READ mofa ovarian cancer
mofa_object.ov <- load_model("mofa_model_ov.hdf5")

# READ mofa Colon cancer
mofa_object.colon <- load_model("mofa_model_colon.hdf5")
...
plot_data_overview(mofa_object.colon)
plot_data_overview(mofa_object.ov)

# Check metadata for both objects
ov_metadata = samples_metadata(mofa_object.ov)
coad_metadata = samples_metadata(mofa_object.colon)

# Plot factor correlations to check for interdependence issues with the model
plot_factor_cor(mofa_object.ov)
plot_factor_cor(mofa_object.colon)


# Plot variance explained
plot_variance_explained(mofa_object.colon)
plot_variance_explained(mofa_object.ov)
...
# Plot data heatmaps
# using rna seq view
plot_data_heatmap(mofa_object.colon,
                   scale = "row",
                   cluster_cols = FALSE,
                   cluster_rows = FALSE,
                   show_colnames = FALSE,
                   denoise = TRUE,
                   features = 10,
                   view = "COAD_RNASeq2GeneNorm-20160128",
                   factor=1)
...
# Plot top weights 
plot_top_weights(mofa_object.colon,
                 view = "slide_assay",
                 factor = 1,
                 nfeatures = 10)
...
# Plot factor scatter
plot_factor(mofa_object.ov, 
            factor = c(1, 2), 
            color_by = "patient.tissue_source_site")
...
coad_metadata$vital_status.x
correlate_factors_with_covariates(mofa_object.colon, 
                                  factors = "all",
                      covariates = c("patient.tissue_source_site", "vital_status.x" ,"pathologic_stage", "patient.age_at_initial_pathologic_diagnosis", "patient.clinical_cqcf.history_of_prior_malignancy"), 
                      plot = "log_pval" )

correlate_factors_with_covariates(mofa_object.ov, 
                                  factors = "all",
                      covariates = c("patient.tissue_source_site", "vital_status" ,"k4", "patient.age_at_initial_pathologic_diagnosis"), 
                      plot = "log_pval" )
...
plot_factor( mofa_object.ov,
             dodge = TRUE,
             add_violin = TRUE,
             color_by = "patient.age_at_initial_pathologic_diagnosis",
             factors=1:6)
...
plot_top_weights( mofa_object.ov,
                  view = "OV_RNASeq2GeneNorm-20160128",
                  nfeatures = 15,
                  scale = T,
                  factor=1)
...
loadings_rnaseq_list <- get_weights(mofa_object.ov, views = "RNASeq")
...
```{r}
plot_weights(mofa_object.colon, 
             view = 2, 
             factor = 3, 
             nfeatures = 10)
...
table(factor3_cluster_df$patient.tissue_source_site, factor3_cluster_df$cluster, useNA="ifany")
...
# cox table
table(surv_df$cluster, surv_df$event)
table(surv_df$histological_type.x, surv_df$cluster)
...
# Epi table 1 stratified by cluster
library("gtsummary")
library(dplyr)
# ensure factors etc. already defined
# table 1 by cluster
table1_cluster <- surv_df %>%
  select(cluster, age, gender, stage, site, hist_type, time, event) %>%
  tbl_summary(
    by = cluster,
    statistic = list(
...
# diagnostics by cluster
diagnostics_cluster <- factor3_cluster_df %>%
  group_by(cluster) %>%
  summarise(
    n = n(),
    n_dead = sum(patient.vital_status == "dead", na.rm = TRUE),
...
# Save full table as PNG
gtsave(as_gt(table1_cluster),
       filename = "table1_cluster.png",
       vwidth = 1800,
       vheight = 2400)


factor3_cluster_df %>%
  group_by(cluster, patient.tumor_tissue_site, year = patient.year_of_initial_pathologic_diagnosis) %>%
  summarise(
    n = n(),
    median_followup = median(patient.days_to_last_followup, na.rm = TRUE),
    zero_followup = sum(patient.days_to_last_followup <= 0, na.rm = TRUE),
    .groups = "drop"
  )

table(factor3_cluster_df$admin.bcr)

names(factor3_cluster_df)
...
candidate_vars <- c(
  ...
  "patient.biospecimen_cqcf.digital_image_submitted",
  "patient.biospecimen_cqcf.top_slide_submitted"
)

for (v in candidate_vars) {
  print(v)
  print(table(factor3_cluster_df[[v]], factor3_cluster_df$cluster, useNA="ifany"))
}

table(metadata$patient.biospecimen_cqcf.top_slide_submitted)

table(factor3_cluster_df$patient.year_of_initial_pathologic_diagnosis,
      factor3_cluster_df$cluster, useNA="ifany")
```

```{r}
# 1. Examine zero follow-up rows
zero_fup <- factor3_cluster_df %>%
  filter(cluster == 1, patient.days_to_last_followup <= 0 | is.na(patient.days_to_last_followup)) %>%
  select(patient.vital_status, days_to_death.x,
         patient.days_to_last_followup, patient.days_to_last_known_alive, tumor_tissue_site)

nrow(zero_fup)

# 2. Check overlap: how many have alternative time variables?
zero_fup %>%
  summarise(
    has_known_alive = sum(!is.na(patient.days_to_last_known_alive)),
    has_death_time  = sum(!is.na(days_to_death.x))
  )

# 3. Inspect distribution of last known alive times
hist(zero_fup$patient.days_to_last_known_alive)

# 4. Compare with cluster 2 for pattern differences
factor3_cluster_df %>%
  group_by(cluster) %>%
  summarise(
    mean_followup = mean(patient.days_to_last_followup, na.rm = TRUE),
    mean_alive_known = mean(patient.days_to_last_known_alive, na.rm = TRUE)
  )
```
...
```{r}
# 2. Systematic missingness analysis
missingness_analysis <- function(data) {
  # Create missingness indicators
  data <- data %>%
    mutate(
      missing_death = is.na(days_to_death.x),
      missing_followup = is.na(patient.days_to_last_followup),
      zero_followup = patient.days_to_last_followup <= 0,
      short_followup = patient.days_to_last_followup < 365  # < 1 year
    )
  
  # Test association with cluster
  missing_tests <- list(
    death_missing = glm(missing_death ~ cluster, data = data, family = binomial),
    followup_missing = glm(missing_followup ~ cluster, data = data, family = binomial),
    zero_followup_test = glm(zero_followup ~ patient.biospecimen_cqcf.top_slide_submitted, data = data, family = binomial),
    short_followup_test = glm(short_followup ~ cluster, data = data, family = binomial)
  )
...
missing_results <- missingness_analysis(factor3_cluster_df)
lapply(missing_results, summary)
```


* strong association between if top slide image submitted and zero follow up
...
critical_vars <- c(
  "patient.tissue_source_site",           # Different collection sites
  "admin.year_of_dcc_upload",             # Data processing batches
  "admin.month_of_dcc_upload",            # More granular timing
  "patient.samples.sample.is_ffpe",       # Sample processing method
  "patient.biospecimen_cqcf.method_of_sample_procurement", # How samples were obtained
  "patient.tissue_prospective_collection_indicator", # Prospective vs retrospective
  "patient.tissue_retrospective_collection_indicator"
)

# Check each one properly
for (var in critical_vars) {
  if(var %in% names(factor3_cluster_df)) {
    cat("\n=== ", var, " ===\n")
    print(table(factor3_cluster_df[[var]], factor3_cluster_df$cluster, useNA = "ifany"))
  }
}
```
...
```{r}
# Check missingness patterns by tissue source site
missingness_by_site <- factor3_cluster_df %>%
  group_by(patient.tissue_source_site) %>%
  summarise(
    n = n(),
    n_in_cluster1 = sum(cluster == 1),
    prop_cluster1 = n_in_cluster1 / n,
    
    # Missingness metrics
    missing_death = sum(is.na(days_to_death.x)),
    prop_missing_death = missing_death / n,
    
    missing_followup = sum(is.na(patient.days_to_last_followup)),
    prop_missing_followup = missing_followup / n,
    
    zero_followup = sum(patient.days_to_last_followup <= 0, na.rm = TRUE),
    prop_zero_followup = zero_followup / n,
    
    mean_followup = mean(patient.days_to_last_followup, na.rm = TRUE),
    median_followup = median(patient.days_to_last_followup, na.rm = TRUE)
  ) %>%
  arrange(desc(prop_cluster1))

# Look at the top sites (those with highest proportion in Cluster 1)
head(missingness_by_site, 10)


```

* Issue with sites aa and ag, mostly retrospective, sites for Indivumed	dominates cluster 1 and has high missingness and zero follow up

* Notes

- Cluster 2 has 94.4% lower odds of having zero follow-up time

- Each increasing year is associated with higher odds of being in Cluster 2

- Cluster 2 has 95.6% lower odds of having short follow-up

* Cluster 1 is almost entirely retrospective samples

* Cluster 2 is mixed but predominantly prospective
...
# Test 1: Adjust for the real confounders in FULL dataset
cluster_surv_cens <- coxph(Surv(time, event) ~ cluster + age + gender + stage + 
...
# Ovar R2: Factor and clin vars

```{r}
# For both OV and COLON
clin_vars <- c(
  # Batch proxy
  "patient.tissue_source_site",
  
  # Demographics  
  "patient.age_at_initial_pathologic_diagnosis",  # numeric - will use lm directly
  "patient.ethnicity",
  
  # Survival/outcome
  "patient.vital_status",
  "patient.days_to_death",                         # numeric
  "patient.days_to_last_followup",                 # numeric
  "patient.person_neoplasm_cancer_status",         # with/without tumor at followup
  "patient.primary_therapy_outcome_success",       # treatment response
  
  # Pathology
  "patient.neoplasm_histologic_grade",
  "patient.residual_tumor",                        # key for OV: R0/R1/R2 debulking
...
factors_metadata_ov <- get_factors(mofa_object.ov, factors = "all", as.data.frame = TRUE) %>%
  inner_join(metadata_ov, by = c("sample" = "patientID")) %>%
  select(sample, factor, value, any_of(clin_vars)) %>%
  pivot_wider(names_from = factor, values_from = value)

r2_ov <- data.frame()
for (var in clin_vars) {
  if (!var %in% colnames(factors_metadata_ov)) next
  for (f in grep("^Factor", names(factors_metadata_ov), value = TRUE)) {
    y <- as.numeric(factors_metadata_ov[[f]])
    x <- factors_metadata_ov[[var]]
    if (!is.numeric(x)) {
      x_num <- suppressWarnings(as.numeric(x))
      if (mean(is.na(x_num)) < 0.5) x <- x_num
    }
    complete_idx <- !is.na(y) & !is.na(x)
    if (sum(complete_idx) < 10 || length(unique(x[complete_idx])) < 2) next
    r2 <- summary(lm(y ~ x))$r.squared
    r2_ov <- rbind(r2_ov, data.frame(Factor = f, Clinical_Variable = var, R2 = r2))
  }
}

ggplot(r2_ov, aes(
  x = as.numeric(gsub("Factor", "", Factor)),
  y = R2, color = Clinical_Variable, group = Clinical_Variable
)) +
  geom_line(linewidth = 0.7) +
  geom_point(size = 1.2) +
  theme_bw(base_size = 11) +
  scale_x_continuous(breaks = 1:max(as.numeric(gsub("Factor", "", r2_ov$Factor)))) +
  scale_color_manual(values = c(
    "#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00",
    "#A65628", "#F781BF", "#009", "#66C2A5", "#999999",
    "#8DA0CB", "#E78AC3", "#A6D854"
  )) +
  labs(title = "MOFA Factors vs Clinical Variables (OV)",
       x = "MOFA Factor", y = expression(R^2),
       color = "Clinical Variable") +
  theme(legend.position = "bottom", legend.text = element_text(size = 7),
        legend.title = element_blank())
```


# Colon R2: Factor and clin vars

```{r}

factors_metadata_colon <- get_factors(mofa_object.colon, factors = "all", as.data.frame = TRUE) %>%
  inner_join(coad_metadata, by = c("sample" = "patientID")) %>%
  select(sample, factor, value, any_of(clin_vars)) %>%
  pivot_wider(names_from = factor, values_from = value)

r2_colon <- data.frame()
for (var in clin_vars) {
  if (!var %in% colnames(factors_metadata_colon)) next
  for (f in grep("^Factor", names(factors_metadata_colon), value = TRUE)) {
    y <- as.numeric(factors_metadata_colon[[f]])
    x <- factors_metadata_colon[[var]]
    if (!is.numeric(x)) {
      x_num <- suppressWarnings(as.numeric(x))
      if (mean(is.na(x_num)) < 0.5) x <- x_num
    }
    ...
    if (sum(complete_idx) < 10 || length(unique(x[complete_idx])) < 2) next
    r2 <- summary(lm(y ~ x))$r.squared
    r2_colon <- rbind(r2_colon, data.frame(Factor = f, Clinical_Variable = var, R2 = r2))
  }
}
...
ggplot(r2_colon, aes(
  ...
  scale_color_manual(values = c(
    "#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00",
    "#A65628", "#F781BF", "#999999", "#66C2A5", "#00008b",
    "#8DA0CB", "#E78AC3", "#A6D854"
  )) +
  labs(title = "MOFA Factors vs Clinical Variables (colon)",
       x = "MOFA Factor", y = expression(R^2),
       color = "Clinical Variable") +
  theme(legend.position = "bottom", legend.text = element_text(size = 7),
         legend.title = element_blank())
...
```{r}
# ============================================================
# Top embedding dimensions per MOFA factor - OV
# ============================================================

embed_weights_ov <- get_weights(mofa_object.ov,
                                 views = "slide_assay",
                                 factors = "all",
                                 as.data.frame = TRUE)

# Top 20 embedding dims per factor by absolute weight
top_embed_ov <- embed_weights_ov %>%
  group_by(factor) %>%
...
# Heatmap: factors x top embedding dims
# Use top 10 per factor for cleaner viz
top10_dims_ov <- top_embed_ov %>%
  group_by(factor) %>%
  slice_head(n = 10) %>%
  ungroup() %>%
  pull(feature) %>%
...
embed_weights_wide_ov <- embed_weights_ov %>%
  filter(feature %in% top10_dims_ov) %>%
  pivot_wider(names_from = feature, values_from = value, values_fill = 0) %>%
  column_to_rownames("factor")


embd_mat_ov <- embed_weights_wide_ov[, sapply(embed_weights_wide_ov, is.numeric)]

pheatmap(
  as.matrix(embd_mat_ov),
  scale = "none",
  cluster_rows = TRUE,
  cluster_cols = FALSE,
  color = colorRampPalette(c("blue", "white", "red"))(100),
  main = "MOFA weights: Factors x Embedding dimensions (OV)",
  fontsize_row = 9,
  fontsize_col = 7
)

# Print top 5 embedding dims per factor as a table
top5_embed_ov <- embed_weights_ov %>%
  group_by(factor) %>%
  mutate(abs_weight = abs(value)) %>%
  arrange(factor, desc(abs_weight)) %>%
  slice_head(n = 5) %>%
  ungroup() %>%
  select(factor, feature, value, abs_weight)
...
```{r}
# ============================================================
# Top embedding dimensions per MOFA factor - COLON
# ============================================================

embed_weights_colon <- get_weights(mofa_object.colon,
                                    views = "slide_assay",
                                    factors = "all",
                                    as.data.frame = TRUE)

top_embed_colon <- embed_weights_colon %>%
  group_by(factor) %>%
  mutate(abs_weight = abs(value)) %>%
  arrange(factor, desc(abs_weight)) %>%
  slice_head(n = 20) %>%
...
top10_dims_colon <- top_embed_colon %>%
  group_by(factor) %>%
  slice_head(n = 10) %>%
  ungroup() %>%
  pull(feature) %>%
...
embed_weights_wide_colon <- embed_weights_colon %>%
  filter(feature %in% top10_dims_colon) %>%
  pivot_wider(names_from = feature, values_from = value, values_fill = 0) %>%
  column_to_rownames("factor")

embd_mat_colon <- embed_weights_wide_colon[, sapply(embed_weights_wide_colon, is.numeric)]


pheatmap(
  as.matrix(embd_mat_colon),
  scale = "none",
  cluster_rows = TRUE,
  cluster_cols = TRUE,
  color = colorRampPalette(c("blue", "white", "red"))(100),
  main = "MOFA weights: Factors x Embedding dimensions (COLON)",
  fontsize_row = 9,
  fontsize_col = 7
)

top5_embed_colon <- embed_weights_colon %>%
  group_by(factor) %>%
  mutate(abs_weight = abs(value)) %>%
  arrange(factor, desc(abs_weight)) %>%
  slice_head(n = 5) %>%
...
print(top5_embed_colon)
```

```
Embedding dim 342, 107, 23  →  high weight in Factor6
Factor6 RNA weights         →  GSEA enriched: HALLMARK_EMT (NES=2.1, padj<0.001)
Conclusion: Those specific embedding dims likely encode EMT morphology
```
...
# ============================================================
# OV: FGSEA on RNA weights per factor
# ============================================================

all_weights_ov <- get_weights(mofa_object.ov, 
                               views = "OV_RNASeq2GeneNorm-20160128", 
                               factors = "all", 
                               as.data.frame = TRUE)

factor_gsea_ov <- list()

for (factor_id in unique(all_weights_ov$factor)) {
  
  factor_weights <- all_weights_ov[all_weights_ov$factor == factor_id, ]
...
```{r}
# ============================================================
# OV: FGSEA Summary Table
# ============================================================

fgsea_summary_ov <- lapply(names(factor_gsea_ov), function(fid) {
  
  hallmark <- factor_gsea_ov[[fid]]$hallmark_top %>%
    filter(!is.na(pathway)) %>%
    arrange(pval) %>%
    slice_head(n = 5) %>%
    mutate(label = paste0(gsub("HALLMARK_", "", pathway), 
                          " (NES=", round(NES, 2), 
                          ", p=", signif(pval, 2), ")")) %>%
    pull(label) %>%
    paste(collapse = "<br>")
  
  c2 <- factor_gsea_ov[[fid]]$c2_top %>%
    filter(!is.na(pathway)) %>%
    arrange(pval) %>%
    slice_head(n = 3) %>%
  ...
  # Top embedding dims for this factor
  top_embeds <- top_embed_ov %>%
    filter(factor == fid) %>%
    slice_head(n = 5) %>%
    mutate(label = paste0(feature, " (w=", round(abs_weight, 3), ")")) %>%
    pull(label) %>%
    paste(collapse = "<br>")
  
  data.frame(
    Factor          = fid,
    Top_Embeddings  = top_embeds,
    Hallmark_GSEA   = hallmark,
    C2_GSEA         = c2,
    C6_GSEA         = c6,
...
fgsea_summary_ov %>%
  kable(
    ...
    col.names = c("Factor", 
                  ...
                  "C2 Pathways", 
                  "C6 Pathways"),
    caption = "<strong>OV: FGSEA Enriched Pathways per MOFA Factor and Associated Embedding Dimensions</strong>"
  ) %>%
  kable_styling(
  ...
  column_spec(1, bold = TRUE, width = "70px",  background = "#1a5276", color = "white") %>%
  column_spec(2, width = "150px", background = "#f0f3f4") %>%
  column_spec(3, width = "250px") %>%
  column_spec(4, width = "220px") %>%
  column_spec(5, width = "220px") %>%
  row_spec(0, background = "#2e86c1", color = "white", bold = TRUE) %>%
...
```{r}
# ============================================================
# COLON: FGSEA on RNA weights per factor
# ============================================================

all_weights_colon <- get_weights(mofa_object.colon, 
                                  views = "COAD_RNASeq2GeneNorm-20160128",  # update view name if different
                                  factors = "all", 
                                  as.data.frame = TRUE)

factor_gsea_colon <- list()

for (factor_id in unique(all_weights_colon$factor)) {
  
  factor_weights <- all_weights_colon[all_weights_colon$factor == factor_id, ]
  ...
  factor_gsea_colon[[factor_id]] <- list(
    hallmark_top = hallmark_res[order(hallmark_res$pval), ][1:20, ],
    c2_top       = c2_res[order(c2_res$pval),             ][1:20, ],
    c6_top       = c6_res[order(c6_res$pval),             ][1:20, ]
  )
...
# Check
factor_gsea_colon$Factor3$hallmark_top
```
...
```{r}
# ============================================================
# COLON: FGSEA Summary Table
# ============================================================

fgsea_summary_colon <- lapply(names(factor_gsea_colon), function(fid) {
  
  hallmark <- factor_gsea_colon[[fid]]$hallmark_top %>%
    filter(!is.na(pathway)) %>%
    arrange(pval) %>%
    slice_head(n = 5) %>%
    mutate(label = paste0(gsub("HALLMARK_", "", pathway),
  ...
  c2 <- factor_gsea_colon[[fid]]$c2_top %>%
    filter(!is.na(pathway)) %>%
    arrange(pval) %>%
    slice_head(n = 3) %>%
    mutate(label = paste0(pathway,
  ...
  c6 <- factor_gsea_colon[[fid]]$c6_top %>%
    filter(!is.na(pathway)) %>%
    arrange(pval) %>%
    slice_head(n = 3) %>%
    mutate(label = paste0(pathway,
...
fgsea_summary_colon %>%
  kable(
    ...
    col.names = c("Factor",
                  ...
                  "C2 Pathways",
                  "C6 Pathways"),
    caption   = "<strong>COLON: FGSEA Enriched Pathways per MOFA Factor and Associated Embedding Dimensions</strong>"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    full_width  = TRUE,
...
```{r}
# ============================================================
# SURVIVAL DATA PREP — Proper time + censoring construction
# ============================================================

# Metadata

metadata_colon <- samples_metadata(mofa_object.colon)
metadata_ov <- samples_metadata(mofa_object.ov)
#' Correctly build OS time and event indicator
#'
#' Rules:
#'   - os_time  = days_to_death       if dead
#'   - os_time  = days_to_last_followup if alive/censored
#'   - os_event = 1 if dead, 0 if censored
#'   - Drop if BOTH time columns are NA, or time <= 0
#'

make_surv_df <- function(mofa_obj, meta,
                         id_col       = "patientID",
                         death_col    = "patient.days_to_death",
                         followup_col = "patient.days_to_last_followup",
                         status_col   = "patient.vital_status",
                         label        = "") {
  
  df <- get_factors(mofa_obj, factors = "all", as.data.frame = TRUE) %>%
    pivot_wider(names_from = factor, values_from = value) %>%
    inner_join(meta, by = c("sample" = id_col))
  
  n_start <- nrow(df)
  
  df <- df %>%
    mutate(
      status_clean = tolower(trimws(.data[[status_col]])),
      
      os_event = case_when(
        status_clean == "dead"  ~ 1L,
        status_clean == "alive" ~ 0L,
        TRUE                    ~ NA_integer_
      ),
      
      os_time = case_when(
        os_event == 1L ~ suppressWarnings(as.numeric(.data[[death_col]])),
        os_event == 0L ~ suppressWarnings(as.numeric(.data[[followup_col]])),
        TRUE           ~ NA_real_
      ),
      
      # document exclusion reason for audit trail
      exclusion_reason = case_when(
        is.na(os_event)          ~ "ambiguous vital status",
        is.na(os_time)           ~ "missing time",
        os_time == 0 & os_event == 0 ~ "alive with zero follow-up (uninformative)",
        os_time == 0 & os_event == 1 ~ "died day 0 (registration artifact)",
        os_time < 0              ~ "negative follow-up time",
        TRUE                     ~ "retained"
      )
    )
  
  # Print exclusion table before filtering
  cat("\n========== EXCLUSION LOG:", label, "==========\n")
  df %>%
  ...
  final_df <- df %>% filter(exclusion_reason == "retained")
  
  cat("Final analytic n:", nrow(final_df),
      "| Events:", sum(final_df$os_event),
      "| Censored:", sum(final_df$os_event == 0), "\n")
  
  final_df
}

surv_colon <- make_surv_df(mofa_object.colon, metadata_colon, label = "COLON")
surv_ov    <- make_surv_df(mofa_object.ov,    metadata_ov,    label = "OV")
...
```{r}
# ============================================================
# FINAL COHORT SUMMARY TABLE — ready for slide/methods section
# ============================================================

# ============================================================
# FINAL COHORT SUMMARY TABLE — select only needed columns
# to avoid type conflicts between colon and OV metadata
# ============================================================

summarise_cohort <- function(surv_df, cancer_label) {
  surv_df %>%
    select(sample, os_event, os_time) %>%   # only the 3 columns we need
    summarise(
      Cancer           = cancer_label,
      N_total          = n(),
      N_events         = sum(os_event),
      N_censored       = sum(os_event == 0),
      Event_rate_pct   = round(100 * mean(os_event), 1),
      Median_days_dead = median(os_time[os_event == 1]),
      Median_days_alive= median(os_time[os_event == 0]),
      Max_followup     = max(os_time),
      N_lt_5y_dead     = sum(os_event == 1 & os_time <  365.25 * 5),
      N_gte_5y         = sum(os_time >= 365.25 * 5)
    )
}

bind_rows(
  summarise_cohort(surv_colon, "COAD"),
  summarise_cohort(surv_ov,    "OV")
) %>%
  kable(
    format    = "html",
    digits    = 0,
    col.names = c("Cancer", "N", "Deaths", "Censored",
                  "Event rate (%)",
                  "Median days (dead)", "Median days (censored)",
                  "Max follow-up (days)",
                  "Deaths < 5y", "Alive/followed ≥ 5y"),
    caption   = "Survival cohort summary after QC exclusions"
  ) %>%
...
```{r}
# ============================================================
# SURVIVAL ANALYSIS — KM + Cox per MOFA Factor
# Goal: understand which embedding-driven factors associate
#       with overall survival in COAD and OV
# ============================================================
...
# ── 1. OVERALL KM — cohort survival summary ─────────────────

plot_overall_km <- function(surv_df, cancer_label) {
  
  fit <- survfit(Surv(os_time, os_event) ~ 1, data = surv_df)
  ...
  p <- ggsurvplot(
    ...
    palette          = "#2c3e50",
    title            = paste(cancer_label, "— Overall Survival"),
    subtitle         = paste0("n=", nrow(surv_df),
                              " | Events=", sum(surv_df$os_event),
                              " | Censored=", sum(surv_df$os_event == 0)),
    xlab             = "Days",
...
plot_overall_km(surv_colon, "COAD")
plot_overall_km(surv_ov,    "OV")
```


### Cox Uni
```{r}
# ── 2. UNIVARIABLE COX — all factors, continuous ────────────

run_cox_univar <- function(surv_df, cancer_label = "") {
  
  factor_cols <- grep("^Factor", names(surv_df), value = TRUE)
  
  map_dfr(factor_cols, function(f) {
    
    df <- surv_df %>%
      select(os_time, os_event, score = all_of(f)) %>%
      filter(!is.na(score))
    
    fit <- coxph(Surv(os_time, os_event) ~ scale(score), data = df)
    
    tidy(fit, exponentiate = TRUE, conf.int = TRUE) %>%
      mutate(Factor = f, Cancer = cancer_label)
    
  }) %>%
    select(Cancer, Factor,
           HR    = estimate,
           CI_lo = conf.low,
           CI_hi = conf.high,
           p.value) %>%
...
cox_uni_colon <- run_cox_univar(surv_colon, "COAD")
cox_uni_ov    <- run_cox_univar(surv_ov,    "OV")
```

### Cox Mult

```{r}
# ── 3. MULTIVARIABLE COX — adjusted for clinical covariates ─

run_cox_multivar <- function(surv_df, cancer_label = "",
                              covariates = NULL) {
  factor_cols <- grep("^Factor", names(surv_df), value = TRUE)
  
  # keep only covariates present + sufficient variation
  covariates <- covariates %>%
    intersect(names(surv_df)) %>%
    Filter(function(v) {
      x <- surv_df[[v]]
      sum(!is.na(x)) >= 20 && length(unique(na.omit(x))) >= 2
    }, .)
  
  message("Covariates used: ", paste(covariates, collapse = ", "))
  ...
  map_dfr(factor_cols, function(f) {
    ...
    cov_str     <- if (length(covariates) > 0)
      paste("+", paste(intersect(covariates, names(df)), collapse = " + "))
    else ""
    
    formula_str <- paste0("Surv(os_time, os_event) ~ scale(score)", cov_str)
    ...
    tidy(fit, exponentiate = TRUE, conf.int = TRUE) %>%
      ...
      mutate(Factor = f, Cancer = cancer_label, n_used = nrow(df))
    
  }) %>%
    select(Cancer, Factor, n_used,
           HR    = estimate,
           CI_lo = conf.low,
           CI_hi = conf.high,
           p.value) %>%
...
cox_mv_colon <- run_cox_multivar(surv_colon, "COAD",
                                  c("patient.neoplasm_histologic_grade",
                                     "patient.ethnicity",
                                    "patient.age_at_initial_pathologic_diagnosis",
                                    "patient.lymphatic_invasion",
...
cox_mv_ov    <- run_cox_multivar(surv_ov, "OV",
                                   c("patient.neoplasm_histologic_grade",
                                     "patient.ethnicity",
                                    "patient.age_at_initial_pathologic_diagnosis",
                                    "patient.lymphatic_invasion",
...
```{r}
plot_forest <- function(uni_df, mv_df, cancer_label) {
  
  combined <- bind_rows(
    uni_df %>% mutate(Model = "Univariable"),
    mv_df  %>% select(-n_used) %>% mutate(Model = "Multivariable")
  ) %>%
    filter(Cancer == cancer_label) %>%
    mutate(
      Factor_num = as.numeric(gsub("Factor", "", Factor)),
      Factor     = fct_reorder(Factor, Factor_num),
      Model      = factor(Model, levels = c("Univariable", "Multivariable"))
    )
  
  dodge <- position_dodge(width = 0.5)   # works for both points and errorbars
  
  ggplot(combined, aes(x = HR, y = Factor, color = Model, shape = Model)) +
    geom_vline(xintercept = 1, linetype = "dashed", color = "grey50") +
    
    # FIX 1: geom_errorbar with orientation = "y" replaces geom_errorbarh
    ...
    # FIX 2: position_dodge replaces position_dodgev
    geom_point(size = 3, position = dodge) +
    
    scale_x_log10() +
    scale_color_manual(values = c(Univariable   = "#2980b9",
                                  Multivariable = "#c0392b")) +
    scale_shape_manual(values = c(Univariable   = 16,
                                  Multivariable = 17)) +
    labs(
      title    = paste(cancer_label, "— Cox PH: MOFA Factors vs Overall Survival"),
      subtitle = "HR per 1 SD increase in factor score (log scale)\nMultivariable: adjusted for age at diag, ethnicity, grade, site, lymph invasion",
...
plot_forest(cox_uni_colon, cox_mv_colon, "COAD")
plot_forest(cox_uni_ov,    cox_mv_ov,    "OV")
```
...
# PCA of Image Embeddings to Interpret MOFA Factors (Ovarian Cancer)

```{r}
# For OV
data_ov <- get_data(mofa_object.ov)                 # list of matrices, one per view
embed_ov_raw <- data_ov[["slide_assay"]]             # features x samples
embed_mat <- embed_ov_raw[["group1"]]   # features x samples
embed_ov <- t(embed_mat)                          # samples x features


# Reshape factors
factors_wide <- factors_ov %>%
  pivot_wider(id_cols = sample, names_from = factor, values_from = value)
factors_mat <- as.matrix(factors_wide[, -1])
rownames(factors_mat) <- factors_wide$sample

# Align
common <- intersect(rownames(factors_mat), rownames(embed_ov))
factors_ov_aligned <- factors_mat[common, , drop = FALSE]
embed_ov_aligned <- embed_ov[common, , drop = FALSE]
```
...
```{r}
library(reshape2)
# 1. Perform PCA on the scaled embeddings
pca_ov <- prcomp(embed_ov_aligned, center = TRUE, scale. = TRUE)

# 2. Variance explained plot (first 10 PCs)
var_exp <- data.frame(
  PC = factor(paste0("PC", 1:10), levels = paste0("PC", 1:10)),
  Variance = summary(pca_ov)$importance[2, 1:10] * 100
)
ggplot(var_exp, aes(x = PC, y = Variance)) +
  geom_col(fill = "steelblue") +
  labs(title = "Variance Explained by Principal Components (OV)",
       x = "Principal Component", y = "Variance Explained (%)") +
  theme_minimal(base_size = 12)

# 3. Extract PC scores (first 10 components for correlation)
k <- 10
pca_scores <- as.data.frame(pca_ov$x[, 1:k])
colnames(pca_scores) <- paste0("PC", 1:k)
pca_scores$sample <- rownames(pca_scores)

# 4. Prepare factor scores as data frame with sample column
factors_df <- as.data.frame(factors_ov_aligned)
factors_df$sample <- rownames(factors_df)

# 5. Merge PC scores with factor scores
merged_df <- merge(pca_scores, factors_df, by = "sample")

# 6. Compute correlation between PCs and MOFA factors
pc_cols <- paste0("PC", 1:k)
factor_cols <- colnames(factors_ov_aligned)
cor_matrix <- cor(merged_df[, pc_cols], merged_df[, factor_cols], 
                  use = "pairwise.complete.obs")

# 7. Heatmap of correlations
cor_melt <- melt(cor_matrix, varnames = c("PC", "Factor"), value.name = "Correlation")
ggplot(cor_melt, aes(x = Factor, y = PC, fill = Correlation)) +
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(-1, 1)) +
  labs(title = "Correlation between MOFA Factors and Image Embedding PCs (OV)",
       x = "MOFA Factor", y = "Principal Component") +
  theme_minimal(base_size = 12) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# 8. For a factor of interest (e.g., Factor1), find the most correlated PC
factor_of_interest <- "Factor1"
best_pc <- names(which.max(abs(cor_matrix[, factor_of_interest])))
cat("PC most correlated with", factor_of_interest, ":", best_pc, "\n")

# 9. Examine the loadings of that PC to understand which embedding features drive it
loadings_best <- pca_ov$rotation[, best_pc]
top_loadings <- sort(abs(loadings_best), decreasing = TRUE)[1:20]
top_features <- names(top_loadings)
top_loadings_df <- data.frame(
  Embedding_Feature = top_features,
  Loading = loadings_best[top_features],
  Abs_Loading = abs(loadings_best[top_features])
) %>% arrange(desc(Abs_Loading))
print(head(top_loadings_df, 10))

# 10. (Optional) Compare with top MOFA weights for the same factor
#     (assuming you have embedding_weight_table from earlier)
# top_weights_factor1 <- subset(embedding_weight_table, Factor == "Factor1")[1:20, ]

# 11. Scatter plot of PC1 vs PC2 colored by the factor score
#     Add clinical metadata from MOFA for extra annotation
metadata_ov <- samples_metadata(mofa_object.ov)
plot_df <- merged_df %>%
  left_join(metadata_ov, by = "sample")

ggplot(plot_df, aes(x = PC1, y = PC2, color = Factor1)) +
  geom_point(size = 2.5, alpha = 0.8) +
  scale_color_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  labs(title = "PCA of Image Embeddings colored by MOFA Factor1 (OV)",
       x = paste0("PC1 (", round(summary(pca_ov)$importance[2,1]*100, 1), "%)"),
       y = paste0("PC2 (", round(summary(pca_ov)$importance[2,2]*100, 1), "%)"),
       color = "Factor1 score") +
  theme_minimal(base_size = 12)
```
...
```{r}
# Get embedding weights from MOFA
embed_weights_ov <- get_weights(mofa_object.ov, 
                                 views = "slide_assay", 
                                 as.data.frame = TRUE)

# For each factor, what are the top embedding dimensions?
top_embed_weights <- embed_weights_ov %>%
  group_by(factor) %>%
  mutate(abs_weight = abs(value)) %>%
  arrange(factor, desc(abs_weight)) %>%
  slice_head(n = 20)
...
# Get weights for RNA view for the same factors
rna_weights_ov <- get_weights(mofa_object.ov,
                               views = "OV_RNASeq2GeneNorm-20160128",
                               as.data.frame = TRUE)

# Top genes per factor
top_rna_weights <- rna_weights_ov %>%
  group_by(factor) %>%
  mutate(abs_weight = abs(value)) %>%
  arrange(factor, desc(abs_weight)) %>%
  slice_head(n = 50)
