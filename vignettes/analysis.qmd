...
missing_coad <- setdiff(all_cols, colnames(coad_clinical_sub))
for (col in missing_coad) {
  coad_clinical_full[[col]] <- NA
}

# Start with READ
read_clinical_full <- read_clinical_sub
missing_read <- setdiff(all_cols, colnames(read_clinical_sub))
for (col in missing_read) {
  read_clinical_full[[col]] <- NA
}

# Now you can rbind safely
clinical_sub <- rbind(coad_clinical_full, read_clinical_full)

# Now subset to patients that actually have embeddings
